...
// Function called by reset button to hide all showing divs
function resetDiv(eleToHide,eleToShow){
    divHide1 = document.getElementById(eleToHide);
    divHide2 = document.getElementById(eleToShow);
...
function validateTextUniversal(textDiv,otherDiv)
{
    ...
    msg = "Required fields are: \n";
    var Flag = 0;

    // Loops through each type=text input and validates.
    for(var i = 0 ; i < textBoxes.length; i++)
    {
        if( textBoxes[i].classList.contains("required_input") )
        {
    ...
    {
        if( intBoxes[k].classList.contains("required_input") )
        {
            x = intBoxes[k].value;
            if(!/^[0-99]+$/.test(x))
            {
